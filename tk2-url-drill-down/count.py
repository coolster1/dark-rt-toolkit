...
def getAllSums(filenames_to_read, col_to_lookup = ''):

    count = 0
    
    for filename in filenames_to_read:
        count = count + getSums(filename, col_to_lookup)

    return count

def getSums(filename_to_read, col_to_lookup = ''):
    ...
    for row in csv_reader:
        ...
        # increment if row has col_to_lookup, else don't
        if row[col_to_lookup]:
            #print col_to_lookup, row[col_to_lookup]
            count = count + int(row[col_to_lookup])
