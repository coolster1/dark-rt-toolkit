def groupRts(prefix, so_far, rtn_type = 'bool', display_filename = False, filename_to_write = '',
             ascii_only = False, extreme_check = False, first_tweet = False, three_check = False, input_folders = list()):
    ...
    if rtn_type == 'three':
        ...
        headers = ['ori', 'P@F', 'P@nF', '@F', '@nF',
                   ...
                   'RtF_d', 'RtnF_d', 'P@F_d', 'P@nF_d', '@F_d', '@nF_d',
        ...
        if os.path.isdir("%s\\%s\\folg_csv" % (prefix, date)):
            ...
            for the_id in os.listdir("%s\\%s\\folg_csv" % (prefix, date)):
                ...
                for url_csv in os.listdir("%s\\%s\\folg_csv\\%s" % (prefix, date, the_id)):
                    ...
                    if (input_folders and filename in input_folders) or (input_folders == list()):

                        # do preliminary check to see filename's not dupes
                        if os.path.isfile(filename):

                            # check if first_tweet matches only
                            if (first_tweet and check_chars.firstTweet("%s\\%s\\streams\\%s\\%s" % (prefix, date, the_id, url_csv), the_id)) or (first_tweet == False):

                                ok = ok + 1

                                files.append(filename)

                                splits = check_chars.countRtsFile([filename], rtn_type, ascii_only, extreme_check, three_check)
