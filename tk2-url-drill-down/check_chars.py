...
def countRtsFile(files, rtn_type = 'split', ascii_only = False, extreme_check = False, three_check = False):
    ...
    splits3 = {'visible': 0 , 'dark': 0, 'orphan': 0, 'ori': 0}
    ...
    for filename in files:
        ...
        if os.path.isfile(filename):
            ...
            for row in csv_reader:
                ...
                if ascii_only:
                    # check majority of row['tweet_text'] is ascii
                    result = chardet.detect(row['tweet_text'])
                    if result['encoding'] == 'ascii' and result['confidence'] > 0.8:
                        check_row = True
                    #print result['encoding'], row['tweet_text']
                else:
                    check_row = True
                ...
                if check_row:
                    ...
                    splits3[isWhichRt(the_key)] = splits3[isWhichRt(the_key)] + 1
    ...
    if three_check:
        ...
        ori_count = (splits3['ori'] if 'ori' in splits3 else 0)
        vis_count = (splits3['visible'] if 'visible' in splits3 else 0)
        dark_count = (splits3['dark'] if 'dark' in splits3 else 0)
        total_rts = vis_count + dark_count
        total_tweets = total_rts + ori_count
        
        # print ori vs rts pct
        if total_tweets > 0 and total_rts > 0:
            ori_pct = ori_count / float(total_tweets) * 1.0
            rts_pct = total_rts / float(total_tweets) * 1.0
            
            # print visible vs dark pct
            vis_pct = vis_count / float(total_rts) * 1.0
            dark_pct = dark_count / float(total_rts) * 1.0
            ...
            if rts_pct < 0.8:
                if dark_pct < 0.8:
                    if total_tweets < 1000:
                        ...
                        if rtn_type == 'split':
                            return {'ori': 0, 'P@F': 0, 'P@nF': 0, '@F': 0, '@nF': 0,
                                    'PRtF': 0, 'PRtnF': 0, 'Rt@F': 0, 'Rt@nF': 0, 'RtF': 0, 'RtnF': 0, 'P@RtF': 0, 'P@RtnF': 0, '@RtF': 0, '@RtnF': 0,
                                    'RtF_d': 0, 'RtnF_d': 0, 'P@F_d': 0, 'P@nF_d': 0, '@F_d': 0, '@nF_d': 0,
                                    'OrphanRt': 0, 'Orphan@': 0}
                        elif rtn_type == 'three':
                            return {'visible': 0 , 'dark': 0, 'orphan': 0, 'ori': 0}
                        elif rtn_type == 'bool':
    ...
    if extreme_check:
        total = splits3['visible'] + splits3['dark'] + splits3['orphan'] + splits3['ori']
        if total > 0:
            vis_pct = float(splits3['visible']) / total
            dark_pct = float(splits3['dark']) / total
            orphan_pct = float(splits3['orphan']) / total
            ori_pct = float(splits3['ori']) / total
        else:
            print "total: 0"
            vis_pct, dark_pct, orphan_pct, ori_pct = 0, 0, 0, 0
                
        if vis_pct >= 0.9:
            print "vis_pct: %.2f" % vis_pct
            ...
            print "orphan_pct: %.2f" % orphan_pct
...
def firstTweet(filename_to_read, the_id):
    ...
    for row in csv_reader:
        if row['author_id'] == the_id:
            to_print.append([row['author_id'], row['tweet_text']])
