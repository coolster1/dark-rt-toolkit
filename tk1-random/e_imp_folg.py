def impFolg(filename_to_read, filename_to_write, friends_dir, id_to_check = ''):#, raw_filename_friends):
...
'''
prefix = "K:\\toolkit_sandbox\\test2"
today = "1710"
author_id = "1179444824"
i = 1
impFolg("%s\\%s\\matrix_csv\\%s\\url_%d.csv" % (prefix, today, author_id, i),
        "%s\\%s\\folg_csv\\%s\\url_%d.csv" % (prefix, today, author_id, i),
        "%s\\users\\friends" % prefix,
        "%s\\%s\\folg_csv\\%s\\seen_friends\\url_%d.csv" % (prefix, today, author_id, i))
#'''
